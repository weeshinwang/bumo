...
describe('static methods', () => {
  it('resolve', () => {
    return Promie.resolve(DEFAULT_VALUE).then((v) =>
      expect(v).toEqual(DEFAULT_VALUE)
    );
  });

  it('reject', () => {
    return Promie.reject(DEFAULT_VALUE).catch((v) =>
      expect(v).toEqual(DEFAULT_VALUE)
    );
  });
  ...
  describe('all', () => {
    it('with success', () => {
      return Promie.all([promise({ value: 1 }), promise({ value: 2 })]).then(
        (v) => expect(v).toEqual([1, 2])
      );
    });

    it('with fail', () => {
      return Promie.all([promise(), promise({ fail: true })]).catch((v) =>
        expect(v).toEqual(DEFAULT_VALUE)
      );
    });
  });
  ...
  it('allSettled', () => {
    return Promie.allSettled([promise(), promise({ fail: true })]).then((v) =>
      expect(v).toEqual([
  ...
  describe('race', () => {
    it('with success', () => {
      return Promie.race([promise({ value: 1 }), promise({ value: 2 })]).then(
  ...
  describe('any', () => {
    it('with success', () => {
      return Promie.any([promise({ value: 1 }), promise({ value: 2 })]).then(
        (v) => expect(v).toEqual(1)
      );
    });
